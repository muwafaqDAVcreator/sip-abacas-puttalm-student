...
<div class="card">
    ...
    <div class="card-body">
        <ul class="nav nav-tabs nav-tabs-highlight">
            ...
            <li class="nav-item"><a href="#edit-record" class="nav-link" data-toggle="tab">Edit Record</a></li>
        </ul>
                                ...
                                <form action="{{ route('payments.pay_now', Qs::hash($uc->payment->id)) }}" method="POST"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <small class="text-muted">You can pay any amount up to
                                            {{ number_format($uc->payment->additional_amount - $additional_payment_paid,
                                            2) }}
                                            LKR.</small>
                                    </div>
   ...
   <i class="icon-printer"></i> Print Receipt
            ...
            <div class="tab-pane fade" id="edit-record">
                <h5 class="mb-3 font-weight-bold">Edit Existing Records</h5>

                @php
                // Build a records collection from existing variables in this view.
                // We only merge $uncleared and $cleared because they have ->payment relation used here.
                $records = collect($uncleared)->merge(collect($cleared));
                @endphp
                ...
                <table class="table table-bordered table-striped table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>Pay Ref</th>
                            <th>Total (LKR)</th>
                            <th>Paid Amount (LKR)</th> {{-- Editable --}}
                            <th>Year</th>
                    ...
                    <tbody>
                        ...
                        @php $hash = Qs::hash($record->id); @endphp
                        <tr>
                            <td>{{ $loop->iteration }}</td>

                            {{-- Title (read-only) --}}
                            <td class="align-middle">{{ $record->payment->title ?? '' }}</td>

                            {{-- Pay Ref (read-only) --}}
                            <td class="align-middle">{{ $record->payment->ref_no ?? '' }}</td>

                            {{-- Total (read-only) --}}
                            <td class="align-middle">{{ number_format($record->payment->amount ?? 0, 2) }}</td>

                            {{-- ✅ Paid Amount (editable) --}}
                            <td>
                                <input type="number" step="0.01" class="form-control form-control-sm"
                                    id="paid-{{ $hash }}" value="{{ $record->amt_paid ?? 0 }}">
                            </td>
                            ...
                            {{-- Actions --}}
                            <td class="text-center">
                                <button type="button" class="btn btn-primary btn-sm update-record"
                                    data-id="{{ $record->id }}" data-hash="{{ $hash }}">
                                    <i class="icon-checkmark4 mr-1"></i> Update
                                </button>
                            </td>
                        </tr>
...
document.querySelectorAll('.update-record').forEach(btn => {
    btn.addEventListener('click', function () {
        const id = this.dataset.id;
        const hash = this.dataset.hash;
        const paidEl = document.getElementById(`paid-${hash}`);
        const paid_amount = paidEl ? parseFloat(paidEl.value || 0) : 0;

        fetch("{{ route('payments.update_record') }}", {
            method: 'POST',
            headers: {
        ...
        .then(async res => {
            ...
            const existingAlert = document.getElementById('paid-success-alert');
            if (existingAlert) existingAlert.remove();

            // Create Bootstrap alert div
            const alertDiv = document.createElement('div');
            alertDiv.className = `alert mt-3 alert-dismissible fade show`;
            alertDiv.role = 'alert';

            if (data.ok) {
                alertDiv.classList.add('alert-success');
                alertDiv.innerHTML = `
                    ${data.message || 'Paid amount updated successfully!'}
            ...
            // Prepend alert to container (replace with your container ID)
            const tabContent = document.getElementById('edit-record') || document.body;
            tabContent.prepend(alertDiv);

            // If update was successful, reload page after 1.5s
            if (data.ok) {
                setTimeout(() => {
                    location.reload();
                }, 1500);
