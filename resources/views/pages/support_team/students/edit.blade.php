...
<div class="card">
    ...
    <form method="post" enctype="multipart/form-data" class="wizard-form steps-validation ajax-update"
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input value="{{ $sr->user->name }}" required type="text" name="name" placeholder="Full Name"
                            class="form-control">
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        <label>ADM_No: <span class="text-danger">*</span></label>
                        <input value="{{ $sr->adm_no }}" required type="text" name="adm_no"
            ...
            <div class="row">
                ...
                {{-- <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select data-placeholder="Choose..." required name="nal_id" id="nal_id"
                            ...
                            <option {{ ($sr->user->nal_id == $na->id ? 'selected' : '') }} value="{{ $na->id }}">{{
                                $na->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                {{-- <div class="col-md-3">
                    ...
                    <select onchange="getLGA(this.value)" required data-placeholder="Choose.."
                        ...
                        <option {{ ($sr->user->state_id == $st->id ? 'selected' : '') }} value="{{ $st->id }}">{{
                            $st->name }}</option>
                        @endforeach
                    </select>
                </div> --}}
                ...
                {{-- <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="select form-control" id="bg_id" name="bg_id" data-fouc
                            ...
                            <option {{ ($sr->user->bg_id == $bg->id ? 'selected' : '') }} value="{{ $bg->id }}">{{
                                $bg->name }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select onchange="getClassSections(this.value)" name="my_class_id" required id="my_class_id"
                            ...
                            <option {{ $sr->my_class_id == $c->id ? 'selected' : '' }} value="{{ $c->id }}">{{ $c->name
                                }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="col-md-3 d-none">
                    <div class="form-group">
                        ...
                        <select data-placeholder="Choose..." name="my_parent_id" id="my_parent_id"
                            ...
                            <option {{ (Qs::hash($sr->parent_id) == Qs::hash($p->id)) ? 'selected' : '' }} value="{{
                ...
                <div class="col-md-3 d-none">
                    <div class="form-group">
                        ...
                        <select name="year_admitted" data-placeholder="Choose..." id="year_admitted"
                            class="select-search form-control">
                            <option value=""></option>
                            @for($y=date('Y', strtotime('- 10 years')); $y<=date('Y'); $y++) <option {{ ($sr->
                                year_admitted == $y) ? 'selected' : '' }} value="{{ $y }}">{{ $y }}</option>
                                @endfor
                        </select>
                    </div>
                </div>
            ...
            {{-- <div class="row">
                <div class="col-md-6">
                    ...
                    <select data-placeholder="Choose..." name="dorm_id" id="dorm_id" class="select-search form-control">
                        <option value=""></option>
                        @foreach($dorms as $d)
                        <option {{ ($sr->dorm_id == $d->id) ? 'selected' : '' }} value="{{ $d->id }}">{{ $d->name }}
                        </option>
                        @endforeach
                    </select>
        ...
        <button type="submit" class="btn btn-primary" style="float: right; margin: 20px;">Updates&nbsp;&nbsp;<i
