...
class UserController extends Controller
{
    ...
    public function update(UserRequest $req, $id)
    {
        ...
        if ($req->hasFile('photo')) {
            ...
            $f = Qs::getFileMetaData($photo);
            $f['name'] = 'photo.' . $f['ext'];
            $f['path'] = $photo->storeAs('public/' . Qs::getUploadPath($user_type) . $data['code'], $f['name']);
            $data['photo'] = asset(str_replace('public/', 'storage/', $f['path']));
        ...
        // redirect to http://127.0.0.1:8000/users this page 
